...
Andere storage of bridge:

```bash
TEMPLATE_STORAGE=local ROOTFS_STORAGE=local-lvm BRIDGE=vmbr0 \
  bash <(curl -fsSL https://raw.githubusercontent.com/AngeloVenneman/proxmox-lxc-bootstrap/main/create-lxc.sh) app01
```
...
2. Controleert de opgegeven template- en rootfs-storage.
3. Bepaalt automatisch een vrije CTID wanneer je er geen opgeeft.
...
16. Toont CTID, hostname, IP en geïnstalleerde versies.
...
> Controleer bij het eerste gebruik of `local`, `local-lvm` en `vmbr0` overeenkomen met jouw Proxmox-configuratie. Deze waarden zijn configureerbaar.